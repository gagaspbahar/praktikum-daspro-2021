...
-- makeUnique menerima masukan sebuah list character lalu menghasilkan list dengan elemen unik.
-- Realisasi
makeUnique l
   | isEmpty l = []
   | otherwise = if numberOfXInL l (last l) == 1 then konsDot (makeUnique (init l)) (last l) else makeUnique (init l)


-- Fungsi Tambahan: numberOfXInL
-- numberOfXInL menerima masukan sebuah list l dan sebuah karakter x lalu menghitung jumlah
-- kemunculan x dalam list tersebut
numberOfXInL :: [Char] -> Char -> Int
numberOfXInL l x
   | isEmpty l = 0
   | otherwise = if head l == x then 1 + numberOfXInL (tail l) x else numberOfXInL (tail l) x 